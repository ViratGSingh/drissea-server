    ...
    const videosWithTranscription = await Promise.all(
      videos.map(async (video) => {
            ...
            messages: [
              {
                role: "user",
                content: [
                  { type: "text", text: "Describe this image vivdly in detail, mention each and every detail and text written in it" },
                  { type: "image_url", image_url: { url: video.video.thumbnail_url } },
                ],
              },
            ],
        ...
        if (!video.has_audio && !video.video?.video_url && (video.video.duration ?? 0) <= 10) {
          ...
          return { ...video, video: { ...video.video, transcription: "", framewatch: framewatchText } };
        }
        try {
          ...
          return {
            ...video,
            video: { ...video.video, transcription: "", framewatch: framewatchText },
            };
        }
      })
    );
