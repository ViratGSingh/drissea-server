...
export class ImagesSerpData extends OpenAPIRoute {
  schema = {
    tags: ["Serp Search"],
  ...
  async handle(c: AppContext) {
    ...
    try {
      ...
      const [
        ...
        imagesRes.json() as Promise<ImagesSerpResponse>,
      ]);

      
      const imagesLinks = (imagesJson?.images_results || []).map((item: any) => ({
        title: item.title,
        source: item.source,
        link: item.link,
        ...
        related_content_id: item.related_content_id,
        serpapi_related_content_link: item.serpapi_related_content_link,
      }));
